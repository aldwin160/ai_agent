...
def generate_content(client, messages, is_verbose=False):
    ...
    function_responses = (
        []
    )  # store the function_response so the LLM can see the function’s output on the next

    for function_call_part in response.function_calls:
        function_call_result = call_function(function_call_part, is_verbose)
        if (
            not function_call_result.parts  # Prevent wrong format or empty parts
            or not function_call_result.parts[
                0
            ].function_response  # Prevent wrong format or empty response
        ):
